...
点击缩略图可切换当前宣传图。文案、截图填充方式和排版按宣传图分别保存，主机框、跨设备辅助机框与主题分别由整组共用。选择双屏或三屏排版时，辅助槽位默认按当前顺序循环使用后续且不重复的截图，也可以手动指定；引用截图删除后会自动回退。还可以在预览中将一张图片拖到另一张图片上直接交换，或将一张本地图片拖入目标边框；本地图片会添加到截图组并立即填入该槽位。截图不足、图片读取失败、文案进入设备安全区或步骤标签溢出时，导出按钮会保持禁用并显示原因。
